package com.icplaza.evm.utils;// Copyright 2017 Google Inc.
//
...
public class Bech32 {
    ...
    public static Bech32Data decode(final String str) throws Exception {
        ...
        for (int i = 0; i < str.length(); ++i) {
            char c = str.charAt(i);
            if (c < 33 || c > 126) throw new Exception("invalid character" + c);
            if (c >= 'a' && c <= 'z') {
                if (upper)
        ...
        if (pos < 1) throw new Exception("Missing human-readable part");
        ...
        for (int i = 0; i < dataPartLength; ++i) {
            ...
            if (CHARSET_REV[c] == -1) throw new Exception("invalid character" + c);
            values[i] = CHARSET_REV[c];
        }
        String hrp = str.substring(0, pos).toLowerCase(Locale.ROOT);
        if (!verifyChecksum(hrp, values)) throw new Exception("invalid Checksum");
        return new Bech32Data(hrp, Arrays.copyOfRange(values, 0, values.length - 6));
    }

    // bellow are old methods
    ...
    private static byte[] encode(int witnessVersion, byte[] witnessProgram) throws Exception {
    ...
    public static String createNewAddressSecp256k1(String mainPrefix, byte[] publickKey) throws Exception {
        ...
        try {
            ...
            addressResult = encode(mainPrefix, bytes);
        } catch (Exception e) {
